      ...
      class AMI
        ...
        class Parser
          ...
          def packet
            ...
            ahn_log.ami.debug "Packet end: #{@__ragel_p}, #{@current_packet.class}, #{action_id.inspect}"
            ahn_log.ami.debug "=====>#{@current_packet[:raw]}<=====" if @current_packet.raw?
            if action_id
          ...
          # have been seen.
          def wait(action)
            ahn_log.ami.debug "Waiting for #{action.action_id.inspect}"
            ...
            @mutex.synchronize do
              @thread = Thread.new do
                ...
                loop do
                  ...
                  # Adjust the pointers.
                  ahn_log.ami.debug "Got #{bytes.length} bytes, #{bytes.inspect}"
                  @__ragel_p = have
                  @__ragel_data[@__ragel_p..@__ragel_p + bytes.size - 1] = bytes
                  @__ragel_pe = @__ragel_p + bytes.size
                  ahn_log.ami.debug "P: #{@__ragel_p} PE: #{@__ragel_pe}"
                  ...
                  if @__ragel_tokstart.nil? or @__ragel_tokstart == 0
                    ...
                    ahn_log.ami.debug "Sliding #{have} from #{@__ragel_tokstart} to 0 (tokend: #{@__ragel_tokend.inspect})"
                    ...
                    @__ragel_tokstart = 0
                    ahn_log.ami.debug "Data: #{@__ragel_data[0..have-1].inspect}"
                  end
                end
                @thread = nil
              end
